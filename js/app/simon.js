/*
original code by Kelly King, http://www.kellyking.me/ as a Simon Says interactive web page
modified by Tim Villabona to behave as intended to run Lamine says: 
*/
define(['jquery'], function($) {
	...
	var Simon = {
		...
		newRound: function() {
			...
			this.sequence.push(this.randomNumber(1,4));
		...
		checkLose: function() {
			...
			//user has successfully completed sequence
			if (this.copy.length === 0 && this.active) {
				var that = this;
				this.deactivateSimonBoard();

				//70% times, have lamine congratulate from 9 phrases
				if (this.randomNumber(1,10) <= 7 && this.mode !== 'light-only'){
				this.waaw(1,7);
				setTimeout(function(){that.newRound();},600);
				}
				//otherwise just move to new round
				else {
				this.newRound();
				}
		...
		waaw: function(lower,upper) {
			var audio = new Audio('sounds/lamine/' + this.randomNumber(lower,upper) + '.mp3');
			audio.play();
		},
		...
		animate: function(sequence) {
			...
			var interval = setInterval(function() {
				...
				//	if (this.randomNumber(1,10) <= 9 && this.mode !== 'light-only'){


				//		that.waaw(10,9);

				setTimeout(function(){that.waaw(10,9);},500);
				//	}
		...
		randomNumber: function(lower,upper) {
			// between lower and lower+upper-1
